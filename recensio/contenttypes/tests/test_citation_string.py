...
class TestCitationString(unittest.TestCase):
    ...
    def test_review_citation_wrt_canonical_uri(self):
        ...
        self.assertEquals('<a href="http://nohost/plone' in monograph_citation, True)

        journal_citation = self.review_jour.get_citation_string()
        self.assertEquals('<a href="http://nohost/plone' in journal_citation, True)

        self.review_mono.canonical_uri = "http://example.com"
        monograph_citation = self.review_mono.get_citation_string()
        self.assertEquals('<a href="http://nohost/plone' in monograph_citation, False)

        self.review_jour.canonical_uri = "http://example.com"
        journal_citation = self.review_jour.get_citation_string()
        self.assertEquals('<a href="http://nohost/plone' in journal_citation, False)
    ...
    def test_page_start_end_in_print(self):
        ...
        self.assertEquals(self.review_mono.page_start_end_in_print, "10-20")
        self.review_mono.pageStartOfReviewInJournal = ""
        self.review_mono.pageEndOfReviewInJournal = "20"
        self.assertEquals(self.review_mono.page_start_end_in_print, "20")
        self.review_mono.pageStartOfReviewInJournal = ""
        self.review_mono.pageEndOfReviewInJournal = ""
        self.assertEquals(self.review_mono.page_start_end_in_print, "")

    def test_doi_link(self):
        # No custom DOI and no canonical_uri: Show UUID URL
        self.review_mono.doi = None
        self.issue_mono.setDoiRegistrationActive(False)
        self.assertNotIn("doi.org", self.review_mono.get_citation_string())
        self.assertIn(
            '<a href="http://nohost/plone/r/', self.review_mono.get_citation_string()
        )
        ...
        self.review_jour.doi = None
        self.issue_jour.setDoiRegistrationActive(False)
        self.assertNotIn("doi.org", self.review_jour.get_citation_string())
        self.assertIn(
            '<a href="http://nohost/plone', self.review_jour.get_citation_string()
        )
        # Custom DOI and no canonical_uri: Show doi.org URL
