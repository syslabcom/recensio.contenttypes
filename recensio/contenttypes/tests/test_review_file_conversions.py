...
class TestReviewFileConversions(unittest.TestCase):
    ...
    def xxxtest_review_with_custom_pdf_files(self):
        ...
        self.assertTrue(
            len(online_resource.pagePictures) > 0
            and len(online_resource.pagePictures[0]) > 1000,
            msg=(
                "Presentation Online Resource: %s "
        ...
        self.assertFalse(
            hasattr(review, "generatedPdf"),
            msg=(
                "Review: %s "
                "A pdf has been generated, even though this "
                "review has a custom pdf (this is wrong)." % review.absolute_url()
            ),
        )
        self.assertTrue(
            review.pdf.blob == review.get_review_pdf()["blob"],
            msg=(
                "Review: %s "
                "get_review_pdf doesn't return the custom pdf." % review.absolute_url()
            ),
        )
        self.assertTrue(
            len(review.pagePictures) > 0 and len(review.pagePictures[0]) > 1000,
            msg=(
                "Review: %s "
                "The generated images for previewing the review "
                "haven't been generated" % (review.absolute_url())
            ),
        )
        ...
        review.setPdf(None)
        self.assertTrue(
            review.getPdf().get_size() == 0,
            msg=(
                "Review: %s "
                "still has an attached custom pdf file, "
                "this should have been removed." % review.absolute_url()
            ),
        )
        ...
        self.assertTrue(
            ...
            msg=(
                "Review: %s "
                "A pdf has not successfully been generated." % review.absolute_url()
            ),
        )
        self.assertTrue(
            review.generatedPdf == review.get_review_pdf()["blob"],
            msg=(
                "Review: %s get_review_pdf "
                "is not returning the correct pdf." % review.absolute_url()
            ),
        )
        ...
        review.setDoc(None)
        self.assertTrue(
            review.getDoc().get_size() == 0,
            msg=(
                "Review: %s "
                "The attached doc file has not been "
                "successfully removed." % review.absolute_url()
            ),
        )
        request = makerequest.makerequest(review)
        event = ObjectEditedEvent(review, request)
